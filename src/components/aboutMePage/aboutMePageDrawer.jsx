...
function AboutMePageDrawer(props) {
  const { window } = props;
  ...
  const drawerElements = [
    ...
    { name: "LinkedIn", icon: <LinkedInIcon />, redirect: aboutMeData.links.linkedin },
    { name: "GitHub", icon: <GitHubIcon />, redirect: aboutMeData.links.github },
  ];
      ...
      <List>
        {drawerElements.map((item) => (
          <ListItem key={item.name} disablePadding>
            <ListItemButton onClick={handleNavigation(item.name)} href={item.redirect}>
              <ListItemIcon>{item.icon}</ListItemIcon>
              <ListItemText primary={item.name} sx={{color: colors.drawerText}} />
            </ListItemButton>
          </ListItem>
        ))}
      </List>
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      >
        <Toolbar/>
        <Typography sx={{ marginBottom: 2, color: colors.generalWebsiteText }}>
...
export default AboutMePageDrawer;
